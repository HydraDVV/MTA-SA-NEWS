function showStats(thePlayer, commandName, targetPlayerName)
	...
	local names = getPlayerName(thePlayer)
	local height = getElementData(thePlayer, "height")
	local weight = getElementData(thePlayer, "weight")

	local info = {}
	if isOverlayDisabled then
	else
		info = {
			{"Merhaba, "..getElementData(thePlayer, "account:username")..""},
			{""},
			{"Karakter İsmi: " .. names .. ""},
			{"Karakter Yaşı: " .. age .. ""},
			{"Karakter Boyu: " .. height .. ""},
			{"Karakter Kilosu: " .. weight .. ""},
	...
	if job == 0 then
		...
		if isOverlayDisabled then
			...
			table.insert(info, {"   - Meslek Leveli: "..joblevel})
			table.insert(info, {"   - Mesai Durumu: "..jobProgress})
		end
	end
	--CARRIED
	...
	if isOverlayDisabled then
		...
		table.insert(info, {""})
		table.insert(info, {"Cüzdan: ₺"..exports.global:formatMoney(money)})
		table.insert(info, {"Bankadaki Para: ₺"..exports.global:formatMoney(bankmoney)})
		table.insert(info, {"Bakiye: "..exports.global:formatMoney(currentGC) .. " ₺ "})
